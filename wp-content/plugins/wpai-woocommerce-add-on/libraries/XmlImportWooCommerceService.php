...
final class XmlImportWooCommerceService {
    ...
    protected function __construct() {
        try {
            ...
            $taxonomies = array('post_format', 'product_type', 'product_shipping_class');
    ...
    public function syncVariableProductData($parentID) {
        ...
        if (!empty($variations)) {
            ...
            if (count($variationIDs) > 1 || !$this->getImport()->options['make_simple_product']) {
    ...
    public function maybeMakeProductSimple($product, $variationIDs) {
        ...
        if (!empty($parsedData)) {
            ...
            if (isset($parsedData['original_sku'])) {
